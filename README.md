...
### 削峰：讓 Redis 擋掉必定失敗的請求，代價是引入第二個真相來源 ⭐

到 M5 為止，**MySQL 一直是唯一的真相來源**。名額夠不夠只有一個答案，所以前面所有的討論都
不必處理「兩份資料不一致」。

M6 打破這件事：在扣 `joined_count` 之前先問 Redis 一句「還有位子嗎」，沒有就直接回 409，
**連一句寫入 SQL 都不產生**。這是搶票系統的經典架構，而它的代價是明確的 —— 現在有兩份
「還剩幾個名額」的答案，它們一定會漂移。
...
| 漂移方向 | 後果 | 嚴重性 |
|---|---|---|
| 閘門比實際**寬鬆**（以為有位子，其實沒有） | 多放幾個請求進 MySQL，被條件式 `UPDATE` 擋下 | 無害，只是削峰效果變差 |
| 閘門比實際**嚴格**（以為滿了，其實還有） | **明明有位子卻拒絕使用者** | 真正的 bug |

**超賣仍然不可能** —— MySQL 那層一行都沒有動。閘門唯一能造成的傷害是誤殺，而且誤殺
**不會有任何錯誤訊息**：沒有例外、沒有紅字、沒有告警，只有使用者在客服信箱裡說「明明還有
名額啊」。

這個不對稱決定了後面每一個選擇：

```
閘門不存在 / Redis 不可達 / 功能關掉  → 放行（fail-open），不是擋下
建立閘門                              → SET NX，不覆蓋別人已經扣過的值
歸還到已過期的閘門                    → 不做事，不憑空造一個沒有 TTL 的 key
對帳                                  → 直接覆蓋就好，因為它只會往寬鬆偏
```

#### 歸還可以放進交易裡，佔用不行

接線時 `IdempotencyKeyTest` 紅了一支，而那不是測試的問題 —— 它剛好走了一條**繞過 controller
的取消路徑**（直接呼叫 `$activity->cancel()`），把設計的洞照了出來：

```
1. $activity->join($occupant)     ← 直接呼叫模型
2. HTTP 報名 → 閘門 = 0 → 擋下 → 409     ✅ 正確
3. $activity->cancel($occupant)   ← 又是直接呼叫模型
                                     名額還回去了，但【閘門完全不知道】
4. HTTP 重試 → 閘門仍是 0 → 409          ❌ 明明有位子
```

把歸還放在 controller，等於假設所有人都走 controller。壓測指令、console、未來的程式碼都直接
呼叫模型 —— 漏掉一條路徑就是永久誤殺。

修法用的正是上面那個不對稱：

> **歸還放進 `Activity::releaseSeat()`**（`joined_count` 真正減少的那一行）。它多做了會讓閘門
> 變寬鬆 —— 交易後來 rollback、或同一個名額被還兩次，後果都是多幾個請求打到 MySQL 被擋下。
>
> **佔用不能放進去。** 交易 rollback 會讓 `DECR` 留在原地，閘門變嚴格 —— 那會誤殺。所以預扣
> 留在 controller，並在 `catch` 裡歸還。

#### 量到的削峰效果，以及一個嚴重低估效益的指標

100 人搶 10 個名額，每種設定跑三輪：

| | DB 語句 | **列鎖等待次數** | **列鎖等待總時間** |
|---|---|---|---|
| 閘門 off | 680（三輪完全相同） | 97 ~ 99 | 15.6 ~ 23.1 秒 |
| 閘門 on（冷啟動） | 501 ~ 507 | 7 ~ 8 | 0.10 ~ 0.47 秒 |
| 閘門 on（預熱） | 500（三輪完全相同） | 7 ~ 9 | 0.07 ~ 1.5 秒 |

```bash
php artisan gate:check --capacity=10 --racers=100 --off
php artisan gate:check --capacity=10 --racers=100 --warm
```

一開始只量語句數，得到「降 26%」，差點就這樣寫進 README 了。但那個數字**嚴重低估了效益**：

> 被削掉的請求省下的是**動到 `joined_count` 的那兩句** —— 也就是所有報名者都要在活動那一列
> 上排隊的那兩句。剩下四句是 `auth:sanctum` 與 route model binding 的索引查找，它們便宜，
> **而且不互相排隊**。
>
> **資料庫真正稀缺的不是「執行語句的能力」，是那一列上的排隊位置。** 那是整條路徑唯一無法
> 平行化的地方，也正是閘門在保護的東西。列鎖等待降了 92%，等待總時間降了約 96%。

這也順帶解釋了 M1 那個「1088 → 405，降不到零」的底線是由什麼組成的。

#### 兩個沒有兌現的猜測

**預熱沒有明顯優於冷啟動**（只差 1 ~ 7 句）。閘門是懶惰建立的，所以「開賣瞬間」它正好是冷的
—— 我原本以為那會造成明顯的驚群效應。但 `SET NX` 之下，真正落在「閘門還不存在」那個窗口裡
的請求只有少數幾個，在這個規模下小到看不見。

**整場耗時完全沒動**（兩組都 1.6 ~ 1.9 秒）。M4 的結論再次生效：瓶頸在壓測用戶端。所以這裡
**不宣稱閘門讓系統變快**，只說它讓資料庫少排了 92% 的隊。

（第一輪用 200 個並行請求量，得到「冷啟動 701 句、預熱 736 句」，看起來像預熱更糟。實際上
那 35 句約等於多活下來的 9 個請求 —— 200 個行程撞上 `max_connections = 151`，幾十個崩掉，
而崩潰數每次都不一樣。`gate:check` 現在只要有任何行程沒完成就直接 FAIL，拒絕讓那次的數字
被引用。）

#### 我知道它會漂移，這是實測到的數字

`gate:reconcile` 每五分鐘跑一次（掛在 M3 的 `onOneServer()` 底下），拿 MySQL 的數字把閘門
校正回來，並把漂移量分方向記成指標。

在真的三節點叢集上跑六輪併發驗證之後對帳，結果完全符合預測，而且是**可重現的**：

```
活動 302  漂移 +4 個名額（偏嚴格，正在誤殺使用者）
活動 296  漂移 +4 個名額（偏嚴格，正在誤殺使用者）
活動 305  漂移 +4 個名額（偏嚴格，正在誤殺使用者）
（略）
對帳完成：檢查 42 個閘門，6 個有漂移（偏嚴格 24 個名額、偏寬鬆 0 個名額）
```

六輪壓測、六個漂移的閘門、**每一個都剛好 +4**，而偏寬鬆的方向是 0。漂移的那六個活動都是
壓測的「重試風暴」情境：**一位使用者、容量 10、20 個並行重試**。

```
真相 = 9 個空位（只佔到 1 個）
閘門 = 5
來源 = 20 個重試裡有 5 個真的抵達 controller（其餘 15 個被 idempotent:redis 重播掉了）
       1 個合法佔到名額
       4 個扣了 token 卻沒佔到 —— join() 看到「已經報名過」就直接回傳，計數器沒動
```

**這條路徑我在寫程式的時候就知道，而且刻意沒有修。** 要修得改 `Activity::join()` 的回傳型別
（讓它回報「這次有沒有真的佔到名額」）與所有呼叫端。選擇不修的理由是：

- 漂移**有界**：上限就是穿過冪等層的重複請求數
- 方向**已知**，而且**指標會顯示它**（`gate_drift_total{direction="too_strict"}`）
- **五分鐘內自癒**，修復延遲就是排程間隔

> 這是 M6 的重點，也是我認為比「我把它修好了」更值得講的事：**一個引入了第二個真相來源的
> 系統，重點不是保證它不漂移 —— 那做不到 —— 而是知道它往哪個方向漂、漂多少、多久修回來，
> 而且有一條線可以指給人看。**

#### 對帳為什麼不需要 CAS、不需要版本號

直覺上「讀 MySQL → 寫 Redis」中間的空隙很危險：這段期間有請求扣了閘門，覆蓋會把那些扣抹掉。

```
t0  閘門 = 3
t1  對帳讀到 MySQL：還有 3 個空位
t2  三個請求各扣一個 → 閘門 = 0
t3  對帳寫入 3       → 那三個扣被抹掉了
```

結果是閘門比實際寬鬆 —— 而寫進去的值就是「資料庫此刻的空位數」，所以它**永遠不可能變得比
真相嚴格**。往安全的方向偏，於是整個併發控制的問題消失了。

**先想清楚錯誤的方向，比想清楚怎麼避免錯誤省事得多。**

代價要說清楚：對帳跑在搶購正中間時，會把當下所有進行中的預扣一起抹掉，讓那一瞬間多放一批
人進來（他們會在 MySQL 被擋下）。五分鐘一次的間隔就是在賭「不會剛好落在那幾秒」。這件事寫
成了一支測試（`an_in_flight_reservation_looks_like_drift_and_is_returned`），而不是一句註解。

#### 又一次「什麼都沒驗證」，這次藏在 Redis 的 API 裡

列出所有閘門必須用 `SCAN` 不能用 `KEYS` —— Redis 是單執行緒的，`KEYS *` 會讓整台伺服器在掃描
期間停止服務所有人。對帳是背景工作卻拖垮前景流量，那是最糟的一種取捨。

但 `SCAN` 有一個更陰險的細節：

```php
$cursor = 0;      // → scan() 回傳 false，看起來像「沒有任何閘門」
$cursor = null;   // → 回傳 18 個 key
```

phpredis 把 `0` 當成「這輪迭代已經結束」。用 `0` 起頭的話，對帳會**每次都回報「檢查了 0 個
閘門，一切正常」**：綠燈、沒有錯誤、什麼都沒做。這是我在這個專案裡撞到的第五、六次同一種
失敗，所以那支測試的註解直接寫了它存在的理由 —— 讓那種沉默變成紅字。變異測試確認：把
`null` 改回 `0`，三支測試同時變紅。

#### 儀表板上那個 `error = 4`

截圖裡「累計結局」有一根很小的 `error` 柱子，值是 4。那不是裝飾，也不是我忘記清掉 —— 它讓
我回頭查了一輪，而查出來的東西剛好把這整份文件的幾條線串起來。

`up` 全程是 1、負載平衡的存取紀錄裡**一筆 5xx 都沒有**。所以那 4 筆不是從對外的埠進來的。
Prometheus 的時間軸指向 16:04，那正是我第一次跑 `gate:check --racers=200` 的時刻 —— 也就是
200 個行程撞上 `max_connections = 151`、被我事後宣告「不可引用」的那幾輪。

有意思的是為什麼是 4 而不是當時報告的 50：

```
gate:check 自己數的「其他」= 54     ← 所有非 201/409 的回應
指標數的 error              = 4     ← 只有真的走完中介層才會被計到
```

差別在執行順序。`auth:sanctum` 排在 `metrics.registration` **之前**，所以連線耗盡而死在認證
階段的請求，在指標中介層跑到之前就已經是 500 了 —— 指標看不到它們。只有 4 個成功通過認證、
在後面才失敗。

三件事因此變得具體：

- **計數器記得你每一次失敗的量測。** 這是 counter 的本質，`rate()` 會顯示它是 16:04 的一次
  尖峰而不是持續問題 —— 但累計圖上它會一直在。我選擇留著它，而不是把 Redis 清掉讓圖好看。
- **指標的位置就是它的視野。** M5 講過「埋在哪一層是資訊決定的」，這裡是它的反面：埋在
  `auth:sanctum` 之後，就永遠看不到死在認證階段的請求。要看那些，得看負載平衡的存取紀錄。
- **儀表板的用處，是讓你注意到一件你沒有在找的事。** 我打開它是為了截圖，不是為了除錯。
